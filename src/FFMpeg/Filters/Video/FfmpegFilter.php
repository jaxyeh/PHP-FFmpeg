...
{   
    ...
    public function apply(Video $video, VideoInterface $format)
    {
        ...
        $scalecmd = $this->detectScale($dimension, $rotatecmd);
...
//        $commands[] = 'faststart';
    ...
    private function detectScale($dimension, $rotation)
    {
        $rotated = false;
        if ($rotation==self::ROTATE_90 || $rotation==self::ROTATE_270)
            $rotated = true;
        ...
        if ($this->padding) {
            ...
            if (null !== $dimension) {
                ...
                $srcAspect = round($dimension->getRatio()->getValue(), 2);
//TODO: Needs fix.
//var_dump("Source =", $srcWidth, $srcHeight, $srcAspect, "Target =",$targetWidth, $targetHeight, $targetAspect);
//die();
                // Determine Scale Metrics
                if ($rotated) {
                    echo "target flipped".PHP_EOL;
                    // Flip Dimension
                ...
                if ($srcHeight <= $targetHeight && $srcWidth <= $targetWidth) {
                    ...
                    echo "width bigger".PHP_EOL;
                    return 'scale='.$newWidth.':trunc\(out_w/a/2\)*2';
                } elseif ($srcHeight > $targetHeight) {
                    echo "height bigger".PHP_EOL;
                    return 'scale=trunc\(out_h/a/2\)*2:'.$newHeight;
                }
            } else {
                return null;
            }
